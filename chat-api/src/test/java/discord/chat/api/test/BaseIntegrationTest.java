...
public class BaseIntegrationTest {
    ...
    // Creates indexes for the test database.
    private void createTestDBIndexes() {
        if (mongoTemplate.getDb().getName().equals(TEST_DB)) {
            ...
            // 모든 엔티티의 인덱스를 가져와 처리
            mongoMappingContext.getPersistentEntities().forEach(mongoEntity -> {
                if (mongoEntity.isAnnotationPresent(Document.class)) {
                    // 해당 엔티티의 컬렉션 이름
                    String collectionName = mongoEntity.getCollection();

                    // 컬렉션에 대해 IndexOperations 가져오기
                    IndexOperations collectionIndexOperations = mongoTemplate.indexOps(collectionName);

                    // 해당 엔티티의 인덱스 정의를 모두 가져와서 적용
                    for (IndexDefinition indexDefinition :
                        indexResolver.resolveIndexFor(mongoEntity.getTypeInformation())) {
                        collectionIndexOperations.ensureIndex(indexDefinition);  // 인덱스 적용
                    }
                }
            });
